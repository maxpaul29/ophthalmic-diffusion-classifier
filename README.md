...
| 10-fold cross-validation for any of the above | as above, plus `CROSS_VALIDATION=1` |
...
- **Deterministic splits**: all dataset splits (hold-out split and 10-fold cross-validation) are generated by versioned scripts in `dataset/splits/`, documented in [SPLITS.md](dataset/splits/SPLITS.md) — re-running a split script with the same input reproduces the same CSVs.
- **Cross-validation everywhere it matters**: fine-tuning, from-scratch training, and the baseline are all evaluated via the same 10-fold procedure over the hold-out split's exact image pool, reported as mean ± standard deviation rather than a single train/valid/test split, given the small size of the private Drusen dataset (111 original images).
