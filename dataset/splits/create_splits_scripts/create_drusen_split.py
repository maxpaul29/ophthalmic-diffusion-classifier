...
(non-drusen) class, matching the paper's balanced-training setup. Use --no-balance
to keep the natural class ratio, or --n-non-drusen / --n-drusen to cap how many
...
def main():
    ...
    parser.add_argument("--n-non-drusen", type=int, default=None,
                        help="Draw only this many non-drusen images before splitting. Default: all.")
    ...
    h_train, h_valid, h_test = split_by_patient(non_drusen_files)
    ...
    print(f"\nDrusen total: {len(drusen_files)}, non-drusen total: {len(non_drusen_files)}")
