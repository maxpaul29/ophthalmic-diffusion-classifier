...
confound of independently re-sampled non-drusen images from the full ~4000-image
directory (see create_drusen_cv_splits.py, which draws a fresh random non-drusen
...
- Non-drusen (target=0) images: instead of scanning --non-drusen-dir, the
  candidate pool is restricted to the union of non-drusen image_name entries
  already present in the three hold-out CSVs. Patient-level grouping
  (patient_key()) and the fold-partitioning logic are otherwise identical.
...
was built. This script only reads non-drusen filenames (never augmented), so
this caveat does not actually apply to the non-drusen side; it is mentioned
here only for completeness since the analogous Drusen-side caveat does NOT
apply either, because Drusen images are still read fresh from --drusen-dir
with full augmentation available for whichever originals a new fold assigns
to train.
...
def main():
    parser = argparse.ArgumentParser(
        description="Create k-fold Drusen CV splits restricted to the hold-out split's exact non-drusen image pool."
    )
    ...
    # ── Non-drusen: restricted to exactly the hold-out split's own pool ────────
    holdout_df = pd.concat([
        pd.read_csv(args.holdout_train),
        pd.read_csv(args.holdout_valid),
        pd.read_csv(args.holdout_test),
    ], ignore_index=True)
    holdout_non_drusen_names = sorted(set(holdout_df[holdout_df.target == 0].image_name))
    if not holdout_non_drusen_names:
        raise ValueError("No non-drusen (target=0) rows found across the three hold-out CSVs.")
    non_drusen_files = [Path(args.data_path) / name for name in holdout_non_drusen_names]
    print(f"Non-drusen pool restricted to hold-out split: {len(non_drusen_files)} images "
          f"(vs. the full --non-drusen-dir, which is NOT read by this script).")
    ...
    if len(non_drusen_patient_ids) < args.k_folds:
        raise ValueError(
            f"Only {len(non_drusen_patient_ids)} distinct non-drusen patients in the hold-out pool, "
    ...
    print(f"Non-drusen originals used (restricted to hold-out pool): {len(non_drusen_files)}")
