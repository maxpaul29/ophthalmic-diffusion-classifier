...
- **create_splits_scripts/create_fundus_split.py**
	- Builds the `fundus-{train,valid,test}` CSVs from `fundus-metadata.csv`, stratified 80/10/10 by class.
	- Run from within `create_splits_scripts/` (it resolves `fundus-metadata.csv` and its output relative to its own location).
